...
public class ArrayPerf4 {
    ...
    public static void main(String[] args) {
        ...
        Chart.of(times, chart -> {
            ...
            chart.title().withText("Native vs Morpheus (Sequential & Parallel) Median Initialisation Times (ex-GC), 5 Million Entries (Sample " + count + ")");
            chart.title().withFont(new Font("Verdana", Font.PLAIN, 15));
            chart.axes().domain().label().withText("Data Type");
            chart.axes().range(0).label().withText("Time (Milliseconds)");
            chart.legend().on();
            chart.orientation().horizontal();
            chart.writerPng(new File("./morpheus-docs/docs/images/native-vs-morpheus-init-times.png"), 845, 400);
            chart.show();
        });
    }
