...
public class ArrayPerf1 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Chart.of(initTimes, chart -> {
            ...
            chart.title().withText("Native Array Median Initialisation Times, 5 Million Entries (Sample " + count + ")");
            chart.title().withFont(new Font("Verdana", Font.PLAIN, 15));
            chart.axes().domain().label().withText("Data Type");
            chart.axes().range(0).label().withText("Time (Milliseconds)");
            chart.legend().off();
            chart.orientation().horizontal();
            chart.writerPng(new File("./morpheus-docs/docs/images/native-array-create-times.png"), 845, 360);
            chart.show();
        });

        Chart.of(gcTimes, chart -> {
            chart.plot(0).withBars(0d);
            chart.title().withText("Native Array Median Garbage Collection Times, 5 Million Entries (Sample " + count + ")");
            chart.title().withFont(new Font("Verdana", Font.PLAIN, 15));
            chart.axes().domain().label().withText("Data Type");
            chart.axes().range(0).label().withText("Time (Milliseconds)");
            chart.legend().off();
            chart.orientation().horizontal();
            chart.writerPng(new File("./morpheus-docs/docs/images/native-array-gc-times.png"), 845, 360);
            chart.show();
        });

        Chart.of(memory, chart -> {
            chart.plot(0).withBars(0d);
            chart.title().withText("Native Array Median Memory Usage, 5 Million Entries (Sample " + count + ")");
            chart.title().withFont(new Font("Verdana", Font.PLAIN, 15));
            chart.axes().domain().label().withText("Data Type");
            chart.axes().range(0).label().withText("Memory Usage (MB)");
            chart.legend().on();
            chart.orientation().horizontal();
            chart.writerPng(new File("./morpheus-docs/docs/images/native-array-memory.png"), 845, 360);
            chart.show();
        });
    }
